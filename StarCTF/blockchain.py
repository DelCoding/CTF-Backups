...
def FLAG():
    return 'Here is your flag: *ctf{5m@rt_c0n7raCt_0n_s1dechAin_546f93250dacb}'
...
class SRC20SmartContract:
    ...
    # 撤回调用（用 StarTokens 转化成 StarCoins）
    def onCall_withdraw(self, tx):

        # by calling this you can convert your StarTokens into StarCoins!
        # 赎回的时候不需要签名
        if len(tx['input']) == 1 and len(tx['output']) == 1 and len(tx['signature']) == 0 and tx['input'][
            0] in self.owned_token_utxos:
            print('撤回调用（用 StarTokens 转化成 StarCoins）')
            # 这意味着您想使用StarTokens在输入utxo中兑换StarCoins
    ...
    # 用 StarCoins 购买 StarTokens
    def onCall_buyTokens(self, utxos, tx):
        print('用 StarCoins 购买 StarTokens')
    ...
    # 进行赎回的时候  from_addr：输入的地址  to_addr: 银行的地址
    def sendTokenAtTx(self, tx, from_addr, to_addr, amount):
        ...
        if to_addr == self.addr:
            ...
            amount = -amount
            # 调换后  from_addr：银行的地址  to_addr: 输入的地址
    ...
    for block in blockchain:
        for tx in block['transactions']:
            for input_utxo_id in tx['input']:
                print('正在删除input id：' + input_utxo_id)
                del utxos[input_utxo_id]

            for utxo in tx['output']:
                print('添加output id：' + utxo['id'])
                utxos[utxo['id']] = utxo
...
def append_block(block, difficulty=int('f' * 64, 16)):
    ...
    if block['prev'] != find_blockchain_tail()['hash']: raise Exception(
    ...
    for tx in block['transactions']:
        ...
        for utxo_id in tx['input']:
            ...
            if utxo_id not in utxos:
                raise Exception("invalid id of input utxo. Input utxo({}) does not exist or it has been consumed.".format(utxo_id))
...
def homepage():
    ...
    addr_info = 'the bank\'s addr: ' + bank_address + ', your addr: ' + hacker_address + ', your privkey: ' + pickle.dumps(
        hacker_privkey).encode('hex')
    ...
    return announcement + ('.<br /><br />\r\n\r\n'.join(
        [view_source_code_link, genesis_block_info, addr_info, balance_info, starcoins_utxo_info, startokens_info,
         blockchain_info]))
